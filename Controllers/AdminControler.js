...
import Admin from '../Models/UserModule.js';
...
export const UserRegister = async (req, res) => {
    ...
    try {
        ...
        const newAdmin = new Admin({Username,Address, email, mobileNumber: hashedmobileNumber });
        await newAdmin.save();
...
export const UserLogin = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ message: "Login Successful!", token , student:Admin });
...
export const UserProfile = async (req, res) => {
    ...
    try {
        const user = await Admin.findById(id); // ✅ Correct findById usage
